import os
import re
import shutil
import json
import logging
import asyncio
import time
from typing import Optional, Dict, List, Tuple, Literal
from moviepy import VideoFileClip, concatenate_videoclips
from PIL import Image
from agents import *
import yaml
from interfaces import *
from langchain.chat_models import init_chat_model
from utils.timer import Timer
from utils.rate_limiter import RateLimiter
import importlib

class Script2VideoPipeline:

    def __init__(
        self,
        chat_model: str,
        mllm_model,
        image_generator,
        video_generator,
        working_dir: str,
    ):
        # Instance variables
        self.chat_model = chat_model
        self.image_generator = image_generator
        self.video_generator = video_generator
        self.mllm_model = mllm_model

        # events - instance variables instead of class variables
        self.character_portrait_events = {}
        self.shot_desc_events = {}
        self.frame_events = {}

        self.character_extractor = CharacterExtractor(chat_model=self.chat_model)
        self.character_portraits_generator = CharacterPortraitsGenerator(image_generator=self.image_generator)
        self.storyboard_artist = StoryboardArtist(chat_model=self.chat_model)
        self.camera_image_generator = CameraImageGenerator(chat_model=self.chat_model, image_generator=self.image_generator, video_generator=self.video_generator)
        self.reference_image_selector = ReferenceImageSelector(chat_model=self.mllm_model)

        self.working_dir = working_dir
        os.makedirs(self.working_dir, exist_ok=True)



    @classmethod
    def init_from_config(
        cls,
        config_path: str,
    ):
        with open(config_path, "r") as f:
            config = yaml.safe_load(f)

        chat_model_args = config["chat_model"]["init_args"]
        chat_model = init_chat_model(**chat_model_args)

        # Create separate rate limiters for each service
        chat_model_rpm = config.get("chat_model", {}).get("max_requests_per_minute", None)
        chat_model_rpd = config.get("chat_model", {}).get("max_requests_per_day", None)
        image_generator_rpm = config.get("image_generator", {}).get("max_requests_per_minute", None)
        image_generator_rpd = config.get("image_generator", {}).get("max_requests_per_day", None)
        video_generator_rpm = config.get("video_generator", {}).get("max_requests_per_minute", None)
        video_generator_rpd = config.get("video_generator", {}).get("max_requests_per_day", None)

        chat_model_rate_limiter = RateLimiter(
            max_requests_per_minute=chat_model_rpm,
            max_requests_per_day=chat_model_rpd
        ) if (chat_model_rpm or chat_model_rpd) else None

        image_rate_limiter = RateLimiter(
            max_requests_per_minute=image_generator_rpm,
            max_requests_per_day=image_generator_rpd
        ) if (image_generator_rpm or image_generator_rpd) else None

        video_rate_limiter = RateLimiter(
            max_requests_per_minute=video_generator_rpm,
            max_requests_per_day=video_generator_rpd
        ) if (video_generator_rpm or video_generator_rpd) else None

        # Display rate limiting configuration
        if chat_model_rate_limiter:
            limits = []
            if chat_model_rpm:
                limits.append(f"{chat_model_rpm} req/min")
            if chat_model_rpd:
                limits.append(f"{chat_model_rpd} req/day")
            print(f"Chat model rate limiting: {', '.join(limits)}")

        if image_rate_limiter:
            limits = []
            if image_generator_rpm:
                limits.append(f"{image_generator_rpm} req/min")
            if image_generator_rpd:
                limits.append(f"{image_generator_rpd} req/day")
            print(f"Image generator rate limiting: {', '.join(limits)}")

        if video_rate_limiter:
            limits = []
            if video_generator_rpm:
                limits.append(f"{video_generator_rpm} req/min")
            if video_generator_rpd:
                limits.append(f"{video_generator_rpd} req/day")
            print(f"Video generator rate limiting: {', '.join(limits)}")

        image_generator_cls_module, image_generator_cls_name = config["image_generator"]["class_path"].rsplit(".", 1)
        image_generator_cls = getattr(importlib.import_module(image_generator_cls_module), image_generator_cls_name)
        image_generator_args = config["image_generator"]["init_args"]
        image_generator_args["rate_limiter"] = image_rate_limiter
        image_generator = image_generator_cls(**image_generator_args)

        video_generator_cls_module, video_generator_cls_name = config["video_generator"]["class_path"].rsplit(".", 1)
        video_generator_cls = getattr(importlib.import_module(video_generator_cls_module), video_generator_cls_name)
        video_generator_args = config["video_generator"]["init_args"]
        video_generator_args["rate_limiter"] = video_rate_limiter
        video_generator = video_generator_cls(**video_generator_args)

        return cls(
            chat_model=chat_model,
            image_generator=image_generator,
            video_generator=video_generator,
            working_dir=config["working_dir"],
        )

    async def __call__(
        self,
        script: str,
        user_requirement: str,
        style: str,
        characters: List[CharacterInScene] = None,
        character_portraits_registry: Optional[Dict[str, Dict[str, Dict[str, str]]]] = None,
    ):
        if characters is None:
            characters = await self.extract_characters(script=script)

            # characters_path = os.path.join(self.working_dir, "characters.json")
            # if os.path.exists(characters_path):
            #     with open(characters_path, "r", encoding="utf-8") as f:
            #         characters = [CharacterInScene.model_validate(c) for c in json.load(f)]
            #     print(f"[SKIP] Loaded {len(characters)} characters from existing file.")
            # else:
            #     print(f"[SEARCH] Extracting characters from script...")
            #     characters = await self.extract_characters(script=script)
            #     with open(characters_path, "w", encoding="utf-8") as f:
            #         json.dump([c.model_dump() for c in characters], f, ensure_ascii=False, indent=4)
            #     print(f"[OK] Extracted {len(characters)} characters from script and saved to {characters_path}.")

        if character_portraits_registry is None:
            character_portraits_registry_path = os.path.join(self.working_dir, "character_portraits_registry.json")
            if os.path.exists(character_portraits_registry_path):
                with open(character_portraits_registry_path, "r", encoding="utf-8") as f:
                    character_portraits_registry = json.load(f)
                print(f"[SKIP] Loaded {len(character_portraits_registry)} character portraits from existing file.")
            else:
                print(f"[SEARCH] Generating character portraits...")
                character_portraits_registry = await self.generate_character_portraits(
                    characters=characters,
                    character_portraits_registry=None,
                    style=style,
                )

                with open(character_portraits_registry_path, "w", encoding="utf-8") as f:
                    json.dump(character_portraits_registry, f, ensure_ascii=False, indent=4)
                print(f"[OK] Generated {len(character_portraits_registry)} character portraits and saved to {character_portraits_registry_path}.")



        # design shots
        storyboard = await self.design_storyboard(
            script=script,
            characters=characters,
            user_requirement=user_requirement,
        )

        # decompose visual descriptions of shots
        shot_descriptions = await self.decompose_visual_descriptions(
            shot_brief_descriptions=storyboard,
            characters=characters,
        )

        # construct camera tree
        camera_tree = await self.construct_camera_tree(
            shot_descriptions=shot_descriptions,
        )

        priority_shot_idxs = [camera.parent_cam_idx for camera in camera_tree if camera.parent_cam_idx is not None]
        tasks = [
            self.generate_frames_for_single_camera(
                camera=camera,
                shot_descriptions=shot_descriptions,
                characters=characters,
                character_portraits_registry=character_portraits_registry,
                priority_shot_idxs=priority_shot_idxs,
            )
            for camera in camera_tree
        ]

        video_tasks = [
            self.generate_video_for_single_shot(
                shot_description=shot_description,
            )
            for shot_description in shot_descriptions
        ]
        tasks.extend(video_tasks)
        await asyncio.gather(*tasks)

        final_video_path = os.path.join(self.working_dir, "final_video.mp4")
        if os.path.exists(final_video_path):
            print(f"[SKIP] Skipped concatenating videos, already exists.")
        else:
            print(f"[CAMERA] Starting concatenating videos...")
            video_clips = [
                VideoFileClip(os.path.join(self.working_dir, "shots", f"{shot_description.idx}", "video.mp4"))
                for shot_description in shot_descriptions
            ]
            final_video = concatenate_videoclips(video_clips)
            final_video.write_videofile(final_video_path, codec="libx264", preset="medium")
            print(f"[OK] Concatenated videos, saved to {final_video_path}.")

        # 从视频中提取第一帧作为封面
        if final_video_path.endswith('.mp4'):
            cover_path = final_video_path.replace('.mp4', '_cover.png')
            try:
                clip = VideoFileClip(final_video_path)
                clip.save_frame(cover_path, t=0)
                clip.close()
                logging.info(f"Cover extracted successfully to {cover_path}")
            except Exception as cover_e:
                logging.warning(f"Failed to extract cover: {cover_e}")

        return final_video_path


    async def generate_frames_for_single_camera(
        self,
        camera: Camera,
        shot_descriptions: List[ShotDescription],
        characters: List[CharacterInScene],
        character_portraits_registry: Dict[str, Dict[str, Dict[str, str]]],
        priority_shot_idxs: List[int],
    ):
        print(f"[CAMERA] [generate_frames_for_single_camera] Camera idx={camera.idx}, parent_shot={camera.parent_shot_idx}, missing_info={camera.missing_info}")
        
        # 1. generate the first_frame of the first shot of the camera
        first_shot_idx = camera.active_shot_idxs[0]
        shot_dir = os.path.join(self.working_dir, "shots", str(first_shot_idx))
        first_shot_ff_path = os.path.join(shot_dir, "first_frame.png")
        first_frame_selector_path = os.path.join(shot_dir, "first_frame_selector_output.json")
        shot_desc_path = os.path.join(shot_dir, "shot_description.json")

        # Check if all required files exist for this shot
        required_files_exist = (
            os.path.exists(first_shot_ff_path) and
            os.path.exists(first_frame_selector_path) and
            os.path.exists(shot_desc_path)
        )

        if required_files_exist:
            print(f"[SKIP] Skipped generating shot {first_shot_idx}, all required files already exist:")
            print(f"  - first_frame.png: {os.path.exists(first_shot_ff_path)}")
            print(f"  - first_frame_selector_output.json: {os.path.exists(first_frame_selector_path)}")
            print(f"  - shot_description.json: {os.path.exists(shot_desc_path)}")
            # Initialize frame_events if not exists
            if first_shot_idx not in self.frame_events:
                self.frame_events[first_shot_idx] = {}
            if "first_frame" not in self.frame_events[first_shot_idx]:
                self.frame_events[first_shot_idx]["first_frame"] = asyncio.Event()
            self.frame_events[first_shot_idx]["first_frame"].set()

        else:
            print(f"[IMAGE] Starting first_frame generation for shot {first_shot_idx}...")
            
            # Initialize frame_events if not exists
            if first_shot_idx not in self.frame_events:
                self.frame_events[first_shot_idx] = {}
            if "first_frame" not in self.frame_events[first_shot_idx]:
                self.frame_events[first_shot_idx]["first_frame"] = asyncio.Event()
            
            available_image_path_and_text_pairs = []

            # Safely get ff_vis_char_idxs
            ff_vis_char_idxs = getattr(shot_descriptions[first_shot_idx], 'ff_vis_char_idxs', None)
            print(f"[SEARCH] ff_vis_char_idxs for shot {first_shot_idx}: {ff_vis_char_idxs}")
            
            if ff_vis_char_idxs:
                for character_idx in ff_vis_char_idxs:
                    try:
                        identifier_in_scene = characters[character_idx].identifier_in_scene
                        if identifier_in_scene in character_portraits_registry:
                            registry_item = character_portraits_registry[identifier_in_scene]
                            for view, item in registry_item.items():
                                available_image_path_and_text_pairs.append((item["path"], item["description"]))
                        else:
                            print(f"[WARN] Character {identifier_in_scene} not found in portraits registry")
                    except Exception as e:
                        print(f"[WARN] Error processing character_idx {character_idx}: {e}")
            
            print(f"[SEARCH] Available image pairs after character processing: {len(available_image_path_and_text_pairs)}")
            
            # Initialize new_camera_image_path to avoid undefined variable error
            new_camera_image_path = None
            
            # generate the first_frame based on the shot_description.ff_desc
            if camera.parent_shot_idx is not None:
                # generate the first_frame based on the transition video
                parent_shot_idx = camera.parent_shot_idx
                await self.frame_events[parent_shot_idx]["first_frame"].wait()
                parent_shot_ff_path = os.path.join(self.working_dir, "shots", f"{parent_shot_idx}", "first_frame.png")
                transition_video_path = os.path.join(self.working_dir, "shots", f"{first_shot_idx}", f"transition_video_from_shot_{parent_shot_idx}.mp4")

                if os.path.exists(transition_video_path):
                    print(f"[SKIP] Skipped generating transition video for shot {first_shot_idx} from shot {parent_shot_idx}, already exists.")
                else:
                    print(f"[IMAGE] Starting transition video generation for shot {first_shot_idx} from shot {parent_shot_idx}...")
                    transition_video_output = await self.camera_image_generator.generate_transition_video(
                        first_shot_visual_desc=shot_descriptions[parent_shot_idx].visual_desc,
                        second_shot_visual_desc=shot_descriptions[first_shot_idx].visual_desc,
                        first_shot_ff_path=parent_shot_ff_path,
                    )
                    transition_video_output.save(transition_video_path)
                    print(f"[OK] Generated transition video for shot {first_shot_idx} from shot {parent_shot_idx}, saved to {transition_video_path}.")

                new_camera_image_path = os.path.join(self.working_dir, "shots", f"{first_shot_idx}", f"new_camera_{camera.idx}.png")
                if os.path.exists(new_camera_image_path):
                    print(f"[SKIP] Skipped generating new camera image for shot {first_shot_idx}, already exists.")
                else:
                    print(f"[IMAGE] Starting new camera image generation for shot {first_shot_idx}...")
                    new_camera_image = self.camera_image_generator.get_new_camera_image(transition_video_path)
                    if new_camera_image is None:
                        print(f"[WARN] Failed to generate new camera image for shot {first_shot_idx}, skipping...")
                    else:
                        new_camera_image.save(new_camera_image_path)
                        print(f"[OK] Generated new camera image for shot {first_shot_idx} (not completed), saved to {new_camera_image_path}.")

                        if new_camera_image_path is not None:
                            available_image_path_and_text_pairs.append(
                                (
                                    new_camera_image_path,
                                    f"The composition and background are correct but some elements may be wrong. The wrong elements should be replaced.\nWrong elements: {camera.missing_info}.\nYou must select this image as the main reference and replace the characters in the image with the provided character portraits. Don't change the background."
                                )
                            )


            # 如果子镜头缺少信息，则需要选择参考图像生成
            ff_selector_output_path = os.path.join(self.working_dir, "shots", f"{first_shot_idx}", "first_frame_selector_output.json")
            
            # Always ensure we have the selector output file
            if os.path.exists(ff_selector_output_path):
                with open(ff_selector_output_path, 'r', encoding='utf-8') as f:
                    ff_selector_output = json.load(f)
                print(f"[SKIP] Loaded existing reference image selection for shot {first_shot_idx}")
            elif camera.parent_shot_idx is None or camera.missing_info is not None:
                # Generate new reference image selection
                print(f"[SEARCH] Selecting reference images for shot {first_shot_idx}...")
                print(f"[SEARCH] Available image pairs count: {len(available_image_path_and_text_pairs)}")
                try:
                    ff_selector_output = await self.reference_image_selector.select_reference_images_and_generate_prompt(
                        available_image_path_and_text_pairs=available_image_path_and_text_pairs,
                        frame_description=shot_descriptions[first_shot_idx].ff_desc
                    )
                    with open(ff_selector_output_path, 'w', encoding='utf-8') as f:
                        json.dump(ff_selector_output, f, ensure_ascii=False, indent=4)
                    print(f"[OK] Generated first_frame_selector_output.json for shot {first_shot_idx}")
                except Exception as e:
                    print(f"[ERROR] ERROR in reference selector: {str(e)}")
                    ff_selector_output = {
                        "reference_image_path_and_text_pairs": available_image_path_and_text_pairs[:1] if available_image_path_and_text_pairs else [],
                        "text_prompt": getattr(shot_descriptions[first_shot_idx], 'ff_desc', None) or "Generate a high-quality image"
                    }
                    with open(ff_selector_output_path, 'w', encoding='utf-8') as f:
                        json.dump(ff_selector_output, f, ensure_ascii=False, indent=4)
                    print(f"[WARN] Created fallback first_frame_selector_output.json")
            else:
                # No parent shot and no missing info - create minimal output
                print(f"[WARN] No parent shot or missing info, creating minimal selector output")
                ff_selector_output = {
                    "reference_image_path_and_text_pairs": available_image_path_and_text_pairs[:1] if available_image_path_and_text_pairs else [],
                    "text_prompt": getattr(shot_descriptions[first_shot_idx], 'ff_desc', None) or "Generate a high-quality cinematic image"
                }
                with open(ff_selector_output_path, 'w', encoding='utf-8') as f:
                    json.dump(ff_selector_output, f, ensure_ascii=False, indent=4)
                print(f"[OK] Created first_frame_selector_output.json (minimal mode)")

            # Safe access to ff_selector_output
            try:
                reference_image_path_and_text_pairs = ff_selector_output.get("reference_image_path_and_text_pairs", [])
                prompt = ff_selector_output.get("text_prompt", getattr(shot_descriptions[first_shot_idx], 'ff_desc', None) or "Generate a high-quality image")
            except Exception as e:
                print(f"[ERROR] ERROR reading ff_selector_output: {e}")
                reference_image_path_and_text_pairs = available_image_path_and_text_pairs[:1] if available_image_path_and_text_pairs else []
                prompt = getattr(shot_descriptions[first_shot_idx], 'ff_desc', None) or "Generate a high-quality image"
                
                if not reference_image_path_and_text_pairs:
                    print(f"[WARN] No reference images available, using available images")
                    reference_image_path_and_text_pairs = available_image_path_and_text_pairs[:1] if available_image_path_and_text_pairs else []
                
                prefix_prompt = ""
                for i, (image_path, text) in enumerate(reference_image_path_and_text_pairs):
                    prefix_prompt += f"Image {i}: {text}\n"
                prompt = f"{prefix_prompt}\n{prompt}"
            
            # Extract reference image paths
            reference_image_paths = [item[0] for item in reference_image_path_and_text_pairs if item[0]]
            
            # Ensure we have a valid prompt
            if not prompt or not prompt.strip():
                prompt = getattr(shot_descriptions[first_shot_idx], 'ff_desc', None) or "Generate a high-quality cinematic image"
                print(f"[WARN] Using fallback prompt: {prompt[:50]}...")
            
            # Generate the first frame
            try:
                print(f"[IMAGE] Generating first_frame for shot {first_shot_idx}, prompt: {prompt[:100]}..., ref_images: {len(reference_image_paths)}")
                ff_image: ImageOutput = await self.image_generator.generate_single_image(
                    prompt=prompt,
                    reference_image_paths=reference_image_paths,
                    size="1600x900",
                )
                ff_image.save(first_shot_ff_path)
                self.frame_events[first_shot_idx]["first_frame"].set()
                print(f"[OK] Generated first_frame for shot {first_shot_idx}, saved to {first_shot_ff_path}.")
            except Exception as e:
                # 检查是否为内容策略违规错误
                error_msg = str(e).lower()
                policy_keywords = ["violat", "policy", "content safety", "not allowed", "inappropriate"]

                if any(keyword in error_msg for keyword in policy_keywords):
                    logging.warning(f"[WARN] 提示词可能违反内容政策，尝试优化... 原错误: {e}")

                    # 调用字符提取器优化提示词
                    optimization_prompt = prompt + " This content may violate our policies. You can try changing the prompt words or changing the image, and then try again"

                    try:
                        characters = await self.character_extractor.extract_characters(optimization_prompt)
                        logging.info(f"[SUCCESS] 成功提取优化后的字符信息: {characters}")

                        # 基于提取结果构建新提示词（这里需要根据实际情况调整）
                        # 示例：简单添加安全修饰词
                        safe_prompt = f"{prompt}, professional photography, artistic, safe for work, family-friendly"

                        # 使用优化后的提示词重试
                        logging.info("[RETRY] 使用优化后的提示词重试生成...")
                        ff_image = await self.image_generator.generate_single_image(
                            prompt=safe_prompt,
                            reference_image_paths=reference_image_paths,
                            size="1600x900",
                        )
                        ff_image.save(first_shot_ff_path)
                        self.frame_events[first_shot_idx]["first_frame"].set()
                        logging.info("[SUCCESS] 优化后生成成功!")

                    except Exception as opt_e:
                        logging.error(f"[ERROR] 提示词优化也失败: {opt_e}")
                        # 这里可以尝试更简单的备用方案
                        if reference_image_paths:
                            try:
                                backup_prompt = "A beautiful, artistic scene suitable for all audiences"
                                ff_image = await self.image_generator.generate_single_image(
                                    prompt=backup_prompt,
                                    reference_image_paths=reference_image_paths,
                                    size="1600x900",
                                )
                                ff_image.save(first_shot_ff_path)
                                self.frame_events[first_shot_idx]["first_frame"].set()
                                logging.info("[SUCCESS] 备用方案生成成功!")
                            except:
                                pass
                        raise opt_e

                else:
                    # 其他类型的错误（网络、超时等）
                    logging.error(f"[ERROR] 图像生成失败（非策略问题）: {e}")
                    raise e


        # 2. generate the following frames of the camera
        priority_tasks = []
        normal_tasks = []

        if shot_descriptions[first_shot_idx].variation_type in ["medium", "large"]:
            lf_vis_char_idxs = getattr(shot_descriptions[first_shot_idx], 'lf_vis_char_idxs', [])
            task = self.generate_frame_for_single_shot(
                shot_idx=first_shot_idx, 
                frame_type="last_frame", 
                first_shot_ff_path_and_text_pair=(first_shot_ff_path, shot_descriptions[first_shot_idx].ff_desc),
                frame_desc=shot_descriptions[first_shot_idx].lf_desc,
                visible_characters=[characters[idx] for idx in lf_vis_char_idxs] if lf_vis_char_idxs else [],
                character_portraits_registry=character_portraits_registry,
            )
            normal_tasks.append(task)

        for shot_idx in camera.active_shot_idxs[1:]:
            ff_vis_char_idxs = getattr(shot_descriptions[shot_idx], 'ff_vis_char_idxs', [])
            first_frame_task = self.generate_frame_for_single_shot(
                    shot_idx=shot_idx, 
                    frame_type="first_frame", 
                    first_shot_ff_path_and_text_pair=(first_shot_ff_path, shot_descriptions[first_shot_idx].ff_desc),
                    frame_desc=shot_descriptions[shot_idx].ff_desc,
                    visible_characters=[characters[idx] for idx in ff_vis_char_idxs] if ff_vis_char_idxs else [],
                    character_portraits_registry=character_portraits_registry,
                )
            if shot_idx in priority_shot_idxs:
                priority_tasks.append(first_frame_task)
            else:
                normal_tasks.append(first_frame_task)


            if shot_descriptions[shot_idx].variation_type in ["medium", "large"]:
                last_frame_task = self.generate_frame_for_single_shot(
                    shot_idx=shot_idx, 
                    frame_type="last_frame", 
                    first_shot_ff_path_and_text_pair=(first_shot_ff_path, shot_descriptions[first_shot_idx].ff_desc),
                    frame_desc=shot_descriptions[shot_idx].lf_desc,
                    visible_characters=[characters[idx] for idx in shot_descriptions[shot_idx].lf_vis_char_idxs],
                    character_portraits_registry=character_portraits_registry,
                )
                normal_tasks.append(last_frame_task)


        await asyncio.gather(*priority_tasks)
        await asyncio.gather(*normal_tasks)



    async def generate_video_for_single_shot(
        self,
        shot_description: ShotDescription,
    ):
        print(f"[VIDEO_GEN] ========== generate_video_for_single_shot called for shot {shot_description.idx} ==========")
        
        shot_dir = os.path.join(self.working_dir, "shots", str(shot_description.idx))
        video_path = os.path.join(shot_dir, "video.mp4")
        first_frame_path = os.path.join(shot_dir, "first_frame.png")
        shot_desc_path = os.path.join(shot_dir, "shot_description.json")
        
        # Check if video and required files exist
        video_exists = os.path.exists(video_path)
        
        # Validate existing video file - delete if corrupted (less than 1MB)
        if video_exists:
            video_size = os.path.getsize(video_path)
            if video_size < 1024 * 1024:  # Less than 1MB is likely corrupted
                print(f"[VIDEO_GEN] Video file too small ({video_size} bytes), deleting and regenerating...")
                try:
                    os.remove(video_path)
                    video_exists = False
                except Exception as e:
                    print(f"[VIDEO_GEN] Failed to delete corrupted video: {e}")
                    video_exists = False
        
        required_files_exist = (
            video_exists and
            os.path.exists(first_frame_path) and
            os.path.exists(shot_desc_path)
        )
        
        print(f"[VIDEO_GEN] Checking if video exists: {video_path}")
        if required_files_exist:
            print(f"[SKIP] Skipped generating video for shot {shot_description.idx}, all required files already exist:")
            print(f"  - video.mp4: {os.path.exists(video_path)}")
            print(f"  - first_frame.png: {os.path.exists(first_frame_path)}")
            print(f"  - shot_description.json: {os.path.exists(shot_desc_path)}")
            return video_path
        
        # In stepwise mode, frame_events may not be initialized, so check files instead
        first_frame_path = os.path.join(self.working_dir, "shots", f"{shot_description.idx}", "first_frame.png")
        last_frame_path = os.path.join(self.working_dir, "shots", f"{shot_description.idx}", "last_frame.png")
        
        print(f"[VIDEO_GEN] first_frame_path: {first_frame_path}")
        print(f"[VIDEO_GEN] last_frame_path: {last_frame_path}")
        print(f"[VIDEO_GEN] first_frame exists: {os.path.exists(first_frame_path)}")
        
        # If first_frame doesn't exist, try to generate it
        if not os.path.exists(first_frame_path):
            print(f"[VIDEO_GEN] [WARN] first_frame.png doesn't exist! Trying to generate...")
            try:
                # Try to find any existing image in the shot directory
                shot_dir = os.path.join(self.working_dir, "shots", f"{shot_description.idx}")
                if os.path.exists(shot_dir):
                    # Look for any PNG file that could be used as reference
                    import glob
                    existing_images = glob.glob(os.path.join(shot_dir, "*.png"))
                    existing_images = [img for img in existing_images if "first_frame" not in img and "last_frame" not in img]
                    if existing_images:
                        # Copy an existing image as first_frame.png temporarily
                        import shutil
                        shutil.copy(existing_images[0], first_frame_path)
                        print(f"[VIDEO_GEN] [SUCCESS] Created first_frame from existing image: {existing_images[0]}")
                    else:
                        # No existing images, try to generate using scene description
                        print(f"[VIDEO_GEN] [INFO] No existing images, attempting to generate first_frame...")
                        ff_desc = getattr(shot_description, 'ff_desc', '') or ''
                        if ff_desc and self.image_generator:
                            from models import ImageOutput
                            try:
                                ff_image: ImageOutput = await self.image_generator.generate_single_image(
                                    prompt=ff_desc,
                                    reference_image_paths=[],
                                    size="1600x900",
                                )
                                ff_image.save(first_frame_path)
                                print(f"[VIDEO_GEN] [SUCCESS] Generated first_frame for shot {shot_description.idx}")
                            except Exception as gen_err:
                                print(f"[VIDEO_GEN] [ERROR] Failed to generate first_frame: {gen_err}")
                                # Allow to continue without first_frame - video will be generated without reference
                                first_frame_path = None
                        else:
                            print(f"[VIDEO_GEN] [WARN] No ff_desc available, cannot generate first_frame")
                            first_frame_path = None
                else:
                    print(f"[VIDEO_GEN] [WARN] Shot directory doesn't exist: {shot_dir}")
                    first_frame_path = None
            except Exception as placeholder_err:
                print(f"[VIDEO_GEN] [ERROR] Failed to handle missing first_frame: {placeholder_err}")
                first_frame_path = None
        
        # Proceed with video generation (allow first_frame_path to be None)
        if first_frame_path is None or not os.path.exists(first_frame_path):
            if first_frame_path:
                print(f"[VIDEO_GEN] WARNING: first_frame still not found at {first_frame_path}")
            print(f"[VIDEO_GEN] Proceeding without first_frame reference...")
        
        print(f"[VIDEO_GEN] first_frame ready! Proceeding...")
        
        # Check last_frame
        variation_type = getattr(shot_description, 'variation_type', 'small')
        
        frame_paths = []
        if first_frame_path and os.path.exists(first_frame_path):
            frame_paths.append(first_frame_path)
        
        # Check if we have last_frame
        if os.path.exists(last_frame_path):
            frame_paths.append(last_frame_path)
            print(f"[VIDEO_GEN] Using last_frame as well")
        else:
            print(f"[VIDEO_GEN] No last_frame found, will use only first_frame")
        
        if not frame_paths:
            print(f"[VIDEO_GEN] WARNING: No reference frames available for video generation!")
        
        print(f"[CAMERA] Starting video generation for shot {shot_description.idx}...")
        
        # Get motion_desc and audio_desc safely
        motion_desc = getattr(shot_description, 'motion_desc', '') or ''
        audio_desc = getattr(shot_description, 'audio_desc', '') or ''
        
        try:
            video_output = await self.video_generator.generate_single_video(
                prompt=motion_desc + "\n" + audio_desc,
                reference_image_paths=frame_paths,
            )
            
            # Validate video output
            if not video_output or not video_output.data:
                raise ValueError("Video output is empty")
            
            # Check if data looks like an error response (e.g., JSON string instead of URL)
            if isinstance(video_output.data, str):
                if video_output.data.startswith('{') or video_output.data.startswith('['):
                    raise ValueError(f"Video generation returned error response: {video_output.data[:200]}")
            
            video_output.save(video_path)
            
            # Verify the saved file is valid
            if os.path.exists(video_path):
                file_size = os.path.getsize(video_path)
                if file_size < 1000:  # Less than 1KB is definitely invalid
                    raise ValueError(f"Generated video file is too small ({file_size} bytes), likely corrupted")
            
        except Exception as e:
            # Try with simpler prompt
            try:
                backup_prompt = "A smooth camera movement through a scenic landscape"
                video_output = await self.video_generator.generate_single_video(
                    prompt=backup_prompt,
                    reference_image_paths=frame_paths,
                )
                
                # Validate
                if not video_output or not video_output.data:
                    raise ValueError("Backup video output is empty")
                    
                video_output.save(video_path)
                
            except Exception as backup_e:
                # Try with just the first frame without any prompt
                video_output = await self.video_generator.generate_single_video(
                    prompt="Cinematic scene",
                    reference_image_paths=[first_frame_path],
                )
                
                # Validate
                if not video_output or not video_output.data:
                    raise ValueError(f"Final backup video output is empty: {backup_e}")
                    
                video_output.save(video_path)
        print(f"[OK] Generated video for shot {shot_description.idx}, saved to {video_path}.")
        return video_path

    async def generate_frame_for_single_shot(
        self,
        shot_idx: int,
        frame_type: Literal["first_frame", "last_frame"],
        first_shot_ff_path_and_text_pair: Tuple[str, str],
        frame_desc: str,
        visible_characters: List[CharacterInScene],
        character_portraits_registry: Dict[str, Dict[str, Dict[str, str]]],
    ) -> ImageOutput:

        shot_dir = os.path.join(self.working_dir, "shots", str(shot_idx))
        frame_image_path = os.path.join(shot_dir, f"{frame_type}.png")
        selector_output_path = os.path.join(shot_dir, f"{frame_type}_selector_output.json")

        # Check if both frame image and selector output exist
        if os.path.exists(frame_image_path) and os.path.exists(selector_output_path):
            print(f"[SKIP] Skipped generating {frame_type} for shot {shot_idx}, already exists with selector output.")
            # Initialize frame_events if not exists
            if shot_idx not in self.frame_events:
                self.frame_events[shot_idx] = {}
            if frame_type not in self.frame_events[shot_idx]:
                self.frame_events[shot_idx][frame_type] = asyncio.Event()
            self.frame_events[shot_idx][frame_type].set()
            return frame_image_path

        else:
            print(f"[IMAGE] Starting {frame_type} generation for shot {shot_idx}...")
            available_image_path_and_text_pairs = []
            
            # Safely process visible characters
            if visible_characters:
                for visible_character in visible_characters:
                    try:
                        identifier_in_scene = visible_character.identifier_in_scene
                        if identifier_in_scene in character_portraits_registry:
                            registry_item = character_portraits_registry[identifier_in_scene]
                            for view, item in registry_item.items():
                                available_image_path_and_text_pairs.append((item["path"], item["description"]))
                        else:
                            print(f"[WARN] Character {identifier_in_scene} not in registry")
                    except Exception as e:
                        print(f"[WARN] Error processing character: {e}")
            
            # Always add first shot reference
            if first_shot_ff_path_and_text_pair:
                available_image_path_and_text_pairs.append(first_shot_ff_path_and_text_pair)

            print(f"[SEARCH] Available images for {frame_type}: {len(available_image_path_and_text_pairs)}")

            selector_output_path = os.path.join(self.working_dir, "shots", f"{shot_idx}", f"{frame_type}_selector_output.json")
            selector_output = None
            
            if os.path.exists(selector_output_path):
                try:
                    with open(selector_output_path, 'r', encoding='utf-8') as f:
                        selector_output = json.load(f)
                    print(f"[SKIP] Loaded existing selector output for {frame_type} of shot {shot_idx}")
                except Exception as e:
                    print(f"[WARN] Error loading selector output: {e}")
            
            if not selector_output:
                try:
                    print(f"[SEARCH] Selecting reference images for {frame_type} frame of shot {shot_idx}...")
                    selector_output = await self.reference_image_selector.select_reference_images_and_generate_prompt(
                        available_image_path_and_text_pairs=available_image_path_and_text_pairs,
                        frame_description=frame_desc
                    )
                    with open(selector_output_path, 'w', encoding='utf-8') as f:
                        json.dump(selector_output, f, ensure_ascii=False, indent=4)
                    print(f"[OK] Created selector output for {frame_type} of shot {shot_idx}")
                except Exception as e:
                    print(f"[ERROR] ERROR in reference selector: {e}")
                    # Create fallback
                    selector_output = {
                        "reference_image_path_and_text_pairs": available_image_path_and_text_pairs[:1] if available_image_path_and_text_pairs else [],
                        "text_prompt": frame_desc or "Generate a high-quality image"
                    }
                    with open(selector_output_path, 'w', encoding='utf-8') as f:
                        json.dump(selector_output, f, ensure_ascii=False, indent=4)
                    print(f"[WARN] Created fallback selector output")

            # Safe access to selector_output
            try:
                reference_image_path_and_text_pairs = selector_output.get("reference_image_path_and_text_pairs", [])
                prompt = selector_output.get("text_prompt", frame_desc or "Generate a high-quality image")
            except Exception as e:
                print(f"[ERROR] ERROR reading selector_output: {e}")
                reference_image_path_and_text_pairs = available_image_path_and_text_pairs[:1] if available_image_path_and_text_pairs else []
                prompt = frame_desc or "Generate a high-quality image"
            
            if not reference_image_path_and_text_pairs:
                reference_image_path_and_text_pairs = available_image_path_and_text_pairs[:1] if available_image_path_and_text_pairs else []
            
            prefix_prompt = ""
            for i, (image_path, text) in enumerate(reference_image_path_and_text_pairs):
                prefix_prompt += f"Image {i}: {text}\n"
            prompt = f"{prefix_prompt}\n{prompt}"
            reference_image_paths = [item[0] for item in reference_image_path_and_text_pairs if item[0]]

            if not prompt or not prompt.strip():
                prompt = frame_desc or "Generate a high-quality cinematic image"

            try:
                print(f"[IMAGE] Generating {frame_type} for shot {shot_idx}, prompt: {prompt[:50]}..., ref_images: {len(reference_image_paths)}")
                frame_image: ImageOutput = await self.image_generator.generate_single_image(
                    prompt=prompt,
                    reference_image_paths=reference_image_paths,
                    size="1600x900",
                )
                frame_image.save(frame_image_path)
                print(f"[OK] Generated {frame_type} frame for shot {shot_idx}, saved to {frame_image_path}.")
            except Exception as e:
                print(f"[ERROR] ERROR generating {frame_type}: {e}")
                # Try with just reference images
                if available_image_path_and_text_pairs:
                    try:
                        backup_prompt = frame_desc or "Generate a high-quality cinematic image"
                        frame_image = await self.image_generator.generate_single_image(
                            prompt=backup_prompt,
                            reference_image_paths=[item[0] for item in available_image_path_and_text_pairs if item[0]],
                            size="1600x900",
                        )
                        frame_image.save(frame_image_path)
                        print(f"[OK] Generated {frame_type} with backup approach")
                    except Exception as backup_e:
                        print(f"[ERROR] Backup also failed: {backup_e}")
                        raise e
                else:
                    raise e

        # Initialize frame_events if not exists
        if shot_idx not in self.frame_events:
            self.frame_events[shot_idx] = {}
        if frame_type not in self.frame_events[shot_idx]:
            self.frame_events[shot_idx][frame_type] = asyncio.Event()
        self.frame_events[shot_idx][frame_type].set()
        return frame_image_path


    async def construct_camera_tree(
        self,
        shot_descriptions: List[ShotDescription],
    ):
        camera_tree_path = os.path.join(self.working_dir, "camera_tree.json")

        if os.path.exists(camera_tree_path):
            with open(camera_tree_path, "r", encoding="utf-8") as f:
                camera_tree = json.load(f)
            camera_tree = [Camera.model_validate(camera) for camera in camera_tree]
            print(f"[SKIP] Loaded {len(camera_tree)} cameras from existing file.")
            return camera_tree

        cameras: List[Camera] = []
        for shot_description in shot_descriptions:
            if shot_description.cam_idx not in [camera.idx for camera in cameras]:
                cameras.append(Camera(idx=shot_description.cam_idx, active_shot_idxs=[shot_description.idx]))
            else:
                cameras[shot_description.cam_idx].active_shot_idxs.append(shot_description.idx)

        camera_tree = await self.camera_image_generator.construct_camera_tree(cameras=cameras, shot_descs=shot_descriptions)
        with open(camera_tree_path, "w", encoding="utf-8") as f:
            json.dump([camera.model_dump() for camera in camera_tree], f, ensure_ascii=False, indent=4)
        print(f"[SUCCESS] Constructed camera tree and saved to {camera_tree_path}.")
        return camera_tree




    async def extract_characters(
        self,
        script: str,
    ):
        save_path = os.path.join(self.working_dir, "characters.json")

        if os.path.exists(save_path):
            with open(save_path, "r", encoding="utf-8") as f:
                data = json.load(f)
            
            # Handle wrapped format: {"content": "...", "type": "text"}
            if isinstance(data, dict) and 'content' in data:
                content = data['content']
                if isinstance(content, str):
                    try:
                        data = json.loads(content)
                    except json.JSONDecodeError:
                        raise ValueError(f"Invalid JSON string in content field: {content[:100]}...")
                else:
                    data = content
            
            # At this point, data should be a list of character dicts
            if not isinstance(data, list):
                raise ValueError(f"Expected list of characters, got {type(data)}")
            
            characters = [CharacterInScene.model_validate(character) for character in data]
            print(f"[SKIP] Loaded {len(characters)} characters from existing file.")
        else:
            characters = await self.character_extractor.extract_characters(script)
            with open(save_path, "w", encoding="utf-8") as f:
                json.dump([character.model_dump() for character in characters], f, ensure_ascii=False, indent=4)
            print(f"[SUCCESS] Extracted {len(characters)} characters from script and saved to {save_path}.")

        for character in characters:
            self.character_portrait_events[character.idx] = asyncio.Event()

        return characters


    async def generate_character_portraits(
        self,
        characters: List[CharacterInScene],
        character_portraits_registry: Optional[Dict[str, Dict[str, Dict[str, str]]]],
        style: str,
    ):
        character_portraits_registry_path = os.path.join(self.working_dir, "character_portraits_registry.json")
        if character_portraits_registry is None:
            if os.path.exists(character_portraits_registry_path):
                with open(character_portraits_registry_path, 'r', encoding='utf-8') as f:
                    character_portraits_registry = json.load(f)
            else:
                character_portraits_registry = {}


        tasks = [
            self.generate_portraits_for_single_character(character, style)
            for character in characters
            if character.identifier_in_scene not in character_portraits_registry
        ]
        if tasks:
            for future in asyncio.as_completed(tasks):
                character_portraits_registry.update(await future)
                with open(character_portraits_registry_path, 'w', encoding='utf-8') as f:
                    json.dump(character_portraits_registry, f, ensure_ascii=False, indent=4)

            print(f"[SUCCESS] Completed character portrait generation for {len(characters)} characters.")
        else:
            print("[SKIP] All characters already have portraits, skipping portrait generation.")
        return character_portraits_registry


    async def generate_portraits_for_single_character(
        self,
        character: CharacterInScene,
        style: str,
    ):
        character_dir = os.path.join(self.working_dir, "character_portraits", f"{character.idx}_{character.identifier_in_scene}")
        os.makedirs(character_dir, exist_ok=True)

        front_portrait_path = os.path.join(character_dir, "front.png")
        if os.path.exists(front_portrait_path):
            pass
        else:
            front_portrait_output = await self.character_portraits_generator.generate_front_portrait(character, style)
            front_portrait_output.save(front_portrait_path)


        side_portrait_path = os.path.join(character_dir, "side.png")
        if os.path.exists(side_portrait_path):
            pass
        else:
            side_portrait_output = await self.character_portraits_generator.generate_side_portrait(character, front_portrait_path)
            side_portrait_output.save(side_portrait_path)

        back_portrait_path = os.path.join(character_dir, "back.png")
        if os.path.exists(back_portrait_path):
            pass
        else:
            back_portrait_output = await self.character_portraits_generator.generate_back_portrait(character, front_portrait_path)
            back_portrait_output.save(back_portrait_path)

        self.character_portrait_events[character.idx].set()

        print(f"[OK] Completed character portrait generation for {character.identifier_in_scene}.")

        return {
            character.identifier_in_scene: {
                "front": {
                    "path": front_portrait_path,
                    "description": f"A front view portrait of {character.identifier_in_scene}.",
                },
                "side": {
                    "path": side_portrait_path,
                    "description": f"A side view portrait of {character.identifier_in_scene}.",
                },
                "back": {
                    "path": back_portrait_path,
                    "description": f"A back view portrait of {character.identifier_in_scene}.",
                },
            }
        }



    async def design_storyboard(
        self,
        script: str,
        characters: List[CharacterInScene],
        user_requirement: str,
    ):
        storyboard_path = os.path.join(self.working_dir, "storyboard.json")
        if os.path.exists(storyboard_path):
            with open(storyboard_path, 'r', encoding='utf-8') as f:
                storyboard = json.load(f)
            storyboard = [ShotBriefDescription.model_validate(shot) for shot in storyboard]
            print(f"[SKIP] Loaded {len(storyboard)} shot brief descriptions from existing file.")
        else:
            print(f"[SEARCH] Designing storyboard...")
            storyboard = await self.storyboard_artist.design_storyboard(
                script=script,
                characters=characters,
                user_requirement=user_requirement,
                retry_timeout=150,
            )
            with open(storyboard_path, 'w', encoding='utf-8') as f:
                json.dump([shot.model_dump() for shot in storyboard], f, ensure_ascii=False, indent=4)
            print(f"[SUCCESS] Designed storyboard and saved to {storyboard_path}.")

        for shot_brief_description in storyboard:
            self.shot_desc_events[shot_brief_description.idx] = asyncio.Event()

        return storyboard



    async def decompose_visual_descriptions(
        self,
        shot_brief_descriptions: List[ShotBriefDescription],
        characters: List[CharacterInScene],
    ):
        tasks = [
            self.decompose_visual_description_for_single_shot_brief_description(shot_brief_description, characters)
            for shot_brief_description in shot_brief_descriptions
        ]

        shot_descriptions = await asyncio.gather(*tasks)
        return shot_descriptions


    async def decompose_visual_description_for_single_shot_brief_description(
        self,
        shot_brief_description: ShotBriefDescription,
        characters: List[CharacterInScene],
    ):
        shot_description_path = os.path.join(self.working_dir, "shots", f"{shot_brief_description.idx}", "shot_description.json")
        os.makedirs(os.path.dirname(shot_description_path), exist_ok=True)

        if os.path.exists(shot_description_path):
            with open(shot_description_path, 'r', encoding='utf-8') as f:
                shot_description = ShotDescription.model_validate(json.load(f))
            print(f"[SKIP] Loaded shot {shot_brief_description.idx} description from existing file.")
        else:
            shot_description = await self.storyboard_artist.decompose_visual_description(
                shot_brief_desc=shot_brief_description,
                characters=characters,
                retry_timeout=120,
            )
            with open(shot_description_path, 'w', encoding='utf-8') as f:
                json.dump(shot_description.model_dump(), f, ensure_ascii=False, indent=4)
            print(f"[SUCCESS] Decomposed visual description for shot {shot_brief_description.idx} and saved to {shot_description_path}.")

        self.shot_desc_events[shot_brief_description.idx].set()

        if shot_description.variation_type in ["medium", "large"]:
            self.frame_events[shot_brief_description.idx] = {
                "first_frame": asyncio.Event(),
                "last_frame": asyncio.Event(),
            }
        else:
            self.frame_events[shot_brief_description.idx] = {
                "first_frame": asyncio.Event(),
            }

        return shot_description

    def _sanitize_motion_description(self, motion_desc: str) -> str:
        sensitive_patterns = [
            (r'\b(nude|naked|explicit|revealing|skimpy|provocative|sexual)\b', 'appropriately dressed'),
            (r'\b(erotic|romantic close-up|intimate|passionate)\b', 'tasteful artistic'),
        ]
        sanitized = motion_desc
        for pattern, replacement in sensitive_patterns:
            sanitized = re.sub(pattern, replacement, sanitized, flags=re.IGNORECASE)
        return sanitized